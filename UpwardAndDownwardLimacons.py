...
class UpwardwardAndDownwardLimacon(Scene):
    def construct(self):
        title = Tex(r"Polar Sketch of")
        title.to_edge(UP)

        polar_curve = Tex(r"Upward and Downward Limacons")
        polar_curve.next_to(title, DOWN, buff=0.3)

        formula = MathTex(r"r = a \pm b\sin\theta").scale(1.2)
        ...
        graph = always_redraw(
            lambda: plane.plot_polar_graph(
                lambda theta: a.get_value() + b.get_value() * np.sin(theta),
                theta_range=[0, TAU],
                color=YELLOW,
            )
        )
        ...
        graph1 = always_redraw(
            lambda: plane.plot_polar_graph(
                lambda theta: a.get_value() - 2 * np.sin(theta),
                theta_range=[0, TAU],
                color=YELLOW,
            )
        )
